...
Interface::Interface(string carDatabase):databaseFile(carDatabase){
	backup(databaseFile);/*		Because this program synchronizes data to file in a clumsy
							method,that is to first remove all old data,then rewrite to it,
							sometimes if before rewrting however after cleaning the program 
							is killed,the data is lost.
								In case this happens,every time the program launches it will 
							first back-up all data to .swap file.
						*/
	readDatabase(databaseFile);
}
Interface::~Interface(){
	sync();//Synchronize data when exiting.
}
...
void Interface::updateCars(){
	...
	while(true){
		...
		if(dictionary.find(hashCode,foundCar)){
			/*
					Here what I have to illustrate is, the find method of the dictionary
				will change the position of its internal curr pointer.
					So if it does find the car,except returning true,it will at the same 
				stop the curr pointer at that found entry.
					Therefore,for this function, every successful update will first remove
				(this no-argument remove function will directly remove the entry that the 
				curr pointer currently points to.) and then execute a sorted insertion.
			*/
...
void Interface::searchCars(){
	while(true){
		...
		if(INVALID_GIST_NUMBER==(gistNumber=getGistNumber()))//Get users' gist number.
			cout<<"无效的查找依据"<<endl;
...
void Interface::sync(){
	fout.open(databaseFile);//Clean the old file.
	if(!fout.is_open()){
	...
	while(!pairList.atEnd()){//Rewrite the file.
...
LList<KVpair<Key,Car>>* Interface::search(searchGist gistNumber){
	string stringGist;
	Date dateGist;
	LList<KVpair<Key,Car>>& pairList=dictionary.getPairList();//Get the reference of all data
	LList<KVpair<Key,Car>>* result=new LList<KVpair<Key,Car>>;;//Create a List to accommodate the result
	KVpair<Key,Car> carGotFromCars;//A temporary container for read data
	while(true){
	clean();
	switch(gistNumber){
		case HASH:
